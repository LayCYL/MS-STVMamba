...
class Dataset_PEMS(Dataset):
    ...
    def __read_data__(self):
        ...
        df_x = df_x.fillna(method="ffill", limit=T_sub).fillna(
            method="bfill", limit=T_sub
        )
        self.data_x = df_x.values.reshape(T_sub, N_sub, C_sub).astype(np.float32)
        df_y = pd.DataFrame(self.data_y)
        df_y = df_y.fillna(method="ffill", limit=len(df_y)).fillna(
            method="bfill", limit=len(df_y)
        )

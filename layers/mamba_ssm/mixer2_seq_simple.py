...
class MixerTSModel(nn.Module):
    def __init__(
        ...
        if self.VPT_mode > 0:
            ...
            assert distance_df_filename is not None, f"When VPT_mode ({self.VPT_mode}) > 0, distance_df_filename should be provided."
        ...
        if self.VPT_mode > 0 and distance_df_filename is not None:

            A, distaneA = get_adjacency_matrix(distance_df_filename, num_of_vertices=n_vars, id_filename=id_filename)

            initial_adj_matrix = torch.tensor(distaneA, dtype=torch.float32, device=device)

            with torch.no_grad():
                self.adjacency_matrix.copy_(initial_adj_matrix)
                
                self.count_matrix.copy_(torch.tensor(A, dtype=torch.float32, device=device) + 1e-7)
            ...
            if self.training:
                ...
                if self.ids_shuffle is None:
                    if self.ATSP_solver == 'GD':
                        ...
                        distance_matrix[:, [start_index, 0]] = distance_matrix[:, [0, start_index]]
                        distance_matrix[[start_index, 0], :] = distance_matrix[[0, start_index], :]
                        if self.ATSP_solver == 'SA':
                            ...
                            raise NotImplementedError(f"Don't support the ATSP solver {self.ATSP_solver}")
